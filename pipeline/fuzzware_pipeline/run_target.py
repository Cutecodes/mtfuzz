...
def gen_run_arglist(config_path, extra_args):
    res = [sys.executable, "-m", "fuzzware_harness.harness", "-c", config_path, "--use_stream","--cov_mode","1", "--cmp_mode","1"] + extra_args
    #res = [sys.executable, "-m", "fuzzware_harness.harness", "-c", config_path, "--use_stream","--cov_mode","1"] + extra_args
    return res
...
def run_target(config_path, input_path, extra_args, get_output=False, silent=False, stdout=None, stderr=None):
    ...
    arg_list = gen_run_arglist(config_path, extra_args) + ["--do_emu", "--input_file", input_path]
    print(arg_list)
    if not silent:
