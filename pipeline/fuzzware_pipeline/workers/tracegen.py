...
class TraceGenProc:
    ...
    def __init__(self, config_path, extra_args=None, gen_bb_set=False, gen_mmio_set=False, gen_bb_hash=False, base_path="/tmp", silent=False):
        ...
        logger.info(f"spawn_emulator_child setting up arguments {extra_args}")
    ...
    def gen_trace(self, input_path, bb_set_path=None, mmio_set_path=None, bb_hash_path=None):
        ...
        run_target(self.config_path, str(input_path), self.extra_args, silent=silent, stdout=subprocess.DEVNULL if silent else None, stderr=subprocess.DEVNULL if silent else None)
