...
{
    uc->fuzzer_init_cov(uc, bitmap_region, bitmap_size, mode);
    return UC_ERR_OK;
}
...
{
    return uc->fuzzer_snapshot_cov(uc);
}

UNICORN_EXPORT
uc_err uc_fuzzer_restore_cov(uc_engine *uc, void* buff)
{
    uc->fuzzer_restore_cov(uc, buff);
    return UC_ERR_OK;
}

UNICORN_EXPORT
uc_err uc_fuzzer_get_cov_prev_loc(uc_engine *uc, void* val){
    *(unsigned long*)val = uc->fuzzer_get_cov_prev_loc(uc);
    return UC_ERR_OK;
}

UNICORN_EXPORT
uc_err uc_fuzzer_set_cov_prev_loc(uc_engine *uc, const void* val){
    uc->fuzzer_set_cov_prev_loc(uc, *(unsigned long*)val);
    return UC_ERR_OK;
}
...
{
    uc->fuzzer_init_cmplog(uc, headers, logs, w, h, mode);
    return UC_ERR_OK;
}
...
{
    uc->fuzzer_reset_cmplog(uc, do_clear);
    return UC_ERR_OK;
}
...
{
    return uc->fuzzer_snapshot_cmplog(uc);
}

UNICORN_EXPORT
uc_err uc_fuzzer_restore_cmplog(uc_engine *uc, void* buff)
{
    uc->fuzzer_restore_cmplog(uc, buff);
    return UC_ERR_OK;
}
...
{
    uc->fuzzer_init_context(uc, sensitive, mode);
    return UC_ERR_OK;
}
...
{
    uc->fuzzer_reset_context(uc, do_clear);
    return UC_ERR_OK;
}
...
{
    return uc->fuzzer_snapshot_context(uc);
}

UNICORN_EXPORT
uc_err uc_fuzzer_restore_context(uc_engine *uc, void* buff)
{
    uc->fuzzer_restore_context(uc, buff);
    return UC_ERR_OK;
}