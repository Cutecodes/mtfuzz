...
static void fuzzer_gen_log_cmp(TCGContext *tcg_ctx, target_ulong pc, TCGv_i32 arg1, TCGv_i32 arg2, enum op_type attr)
{
...
static void fuzzer_log_cmp_rtn(CPUArchState *env, uintptr_t pc)
{ 
    ...
    // Include exec, Fuzzware clears write bit here when not yet written to
    bool m1ro = ((~(m1->perms)) & UC_PROT_WRITE) && (m1->perms & UC_PROT_READ); 
    ...
    register uintptr_t k_loc_extra = ((pc + (string12 ^ string34)) % (cov_area_size)) ^ min_len;
    if (strcmp(lowbytes, highbytes) == 0) {
      if(cov_area_ptr[k_loc_extra] < 255)
        cov_area_ptr[k_loc_extra]++;
    }
    
    register uintptr_t k = pc ;
    k = (k >> 4) ^ (k << 8);
    k = k + (string12 ^ string34);
    k = k ^ min_len;
    k &= cmp_map_w - 1; 
    ...
    //cmp_headers[k].shape = 31;
    cmp_headers[k].shape = lowlen-1;
    cmp_headers[k].type = CMP_TYPE_RTN;
    // invalidComparison may mean length is greater than 30.
    cmp_headers[k].attribute = validComparison;
    if (hits >= cmp_map_h/4) {
        cmp_headers[k].overflow = 1;
    }
    ...
    __builtin_memcpy(((struct cmpfn_operands *)(&cmp_log[k*cmp_map_h]))[hits].v0, (void*)highbytes, readSize);
    __builtin_memcpy(((struct cmpfn_operands *)(&cmp_log[k*cmp_map_h]))[hits].v1, (void*)lowbytes, readSize);
    
    ((struct cmpfn_operands *)(&cmp_log[k*cmp_map_h]))[hits].v0_len = highlen;
    ((struct cmpfn_operands *)(&cmp_log[k*cmp_map_h]))[hits].v1_len = lowlen;
    
}
static void fuzzer_gen_log_cmp_rtn(TCGContext *tcg_ctx, target_ulong pc)
{
    CPUArchState *env = tcg_ctx->cpu_env;
    if (cmp_headers == NULL || cmp_log == NULL) {
        return;
    }
    

    TCGArg args[2] = { GET_TCGV_PTR(env),
                       GET_TCGV_PTR(tcg_const_ptr(tcg_ctx, pc))};
    
    tcg_gen_callN(tcg_ctx, fuzzer_log_cmp_rtn, dh_retvar_void, 2, args);
    return;
}
...
static inline void gen_vfp_cmp(DisasContext *s, int dp)
{
    TCGContext *tcg_ctx = s->uc->tcg_ctx;
    if (dp) {
        fuzzer_gen_log_cmp64(tcg_ctx, s->pc, tcg_ctx->cpu_F0d, tcg_ctx->cpu_F1d, OP_FCMP);
        gen_helper_vfp_cmpd(tcg_ctx, tcg_ctx->cpu_F0d, tcg_ctx->cpu_F1d, tcg_ctx->cpu_env);
    }
    else {
        fuzzer_gen_log_cmp(tcg_ctx, s->pc, tcg_ctx->cpu_F0s, tcg_ctx->cpu_F1s, OP_FCMP);
                   ...
                   TCGv_i32 t0, TCGv_i32 t1)
{
    ...
    case 0: /* and */
        if (conds) {
            fuzzer_gen_log_cmp(tcg_ctx, s->pc, t0, t1, OP_AND);
        }
    ...
    case 2: /* orr */
        if (conds) {
            fuzzer_gen_log_cmp(tcg_ctx, s->pc, t0, t1, OP_OR);
        }
    ...
    case 4: /* eor */
        if (conds) {
            fuzzer_gen_log_cmp(tcg_ctx, s->pc, t0, t1, OP_XOR);
        }
    ...
    case 11: /* sbc */
        if (conds) {
            fuzzer_gen_log_cmp(tcg_ctx, s->pc, t0, t1, OP_ISUB);
    ...
    case 8: case 9: case 10: case 11:
        if (insn & (1 << 15)) {
            ...
            if (insn & 0x5000) {
                ...
                if (insn & (1 << 14)) {
                    /* Branch and link.  */
                    fuzzer_gen_log_cmp_rtn(tcg_ctx, s->pc);
...
static void disas_thumb_insn(CPUARMState *env, DisasContext *s) // qq
{
        ...
        if (op == 3) {
            ...
            if (insn & (1 << 9)) {
                if (s->condexec_mask)
                    ...
                    fuzzer_gen_log_cmp(tcg_ctx, s->pc, tmp, tmp2, OP_ICMP);
                    gen_sub_CC(s, tmp, tmp, tmp2);
                }
            } else {
                if (s->condexec_mask)
                    ...
                    //fuzzer_gen_log_cmp(tcg_ctx, s->pc, tmp, tmp2, OP_IADD);
                    gen_add_CC(s, tmp, tmp, tmp2);
                }
            }
            tcg_temp_free_i32(tcg_ctx, tmp2);
    ...
    case 4:
        ...
        case 0x0: /* and */
            if (!s->condexec_mask) {
                fuzzer_gen_log_cmp(tcg_ctx, s->pc, tmp, tmp2, OP_AND);
            }
            tcg_gen_and_i32(tcg_ctx, tmp, tmp, tmp2);
            if (!s->condexec_mask) {
                gen_logic_CC(s, tmp);
        ...
        case 0x1: /* eor */
            if (!s->condexec_mask) {
                fuzzer_gen_log_cmp(tcg_ctx, s->pc, tmp, tmp2, OP_XOR);
            }
            tcg_gen_xor_i32(tcg_ctx, tmp, tmp, tmp2);
            if (!s->condexec_mask) {
                gen_logic_CC(s, tmp);
        ...
        case 0x6: /* sbc */
            if (s->condexec_mask) {
                ...
                fuzzer_gen_log_cmp(tcg_ctx, s->pc, tmp, tmp2, OP_ISUB);
                gen_sbc_CC(s, tmp, tmp, tmp2);
            }
            break;
        case 0x7: /* ror */
        ...
        case 0xc: /* orr */
            if (!s->condexec_mask) {
                fuzzer_gen_log_cmp(tcg_ctx, s->pc, tmp, tmp2, OP_OR);
            }
            tcg_gen_or_i32(tcg_ctx, tmp, tmp, tmp2);
            if (!s->condexec_mask){
                gen_logic_CC(s, tmp);
