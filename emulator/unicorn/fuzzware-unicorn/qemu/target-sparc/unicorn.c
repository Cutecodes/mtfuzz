        ...
        if (regid >= UC_SPARC_REG_G0 && regid <= UC_SPARC_REG_G7)
            ...
            *(int32_t *)value = SPARC_CPU(uc, mycpu)->env.regwptr[regid - UC_SPARC_REG_O0];
            ...
            switch(regid) {
                ...
                // fuzz add regs
                case UC_SPARC_REG_NPC:
                    *(int32_t *)value = SPARC_CPU(uc, mycpu)->env.npc;
                    break;
                case UC_SPARC_REG_PSR:
                    *(int32_t *)value = cpu_get_psr(&(SPARC_CPU(uc, mycpu)->env));
                    break;
                case UC_SPARC_REG_WIM:
#if !defined(TARGET_SPARC64) || defined(TARGET_ABI32)
                    *(int32_t *)value = SPARC_CPU(uc, mycpu)->env.wim;
#else
                    *(int32_t *)value = 0;
#endif
                    break;
                case UC_SPARC_REG_TBR:
                    *(int32_t *)value = SPARC_CPU(uc, mycpu)->env.tbr;
                    break;
                case UC_SPARC_REG_FSR:
                    *(int32_t *)value = SPARC_CPU(uc, mycpu)->env.fsr;
                    break;
            }
        }
    }
    ...
    for (i = 0; i < count; i++) {
        ...
        if (regid >= UC_SPARC_REG_G0 && regid <= UC_SPARC_REG_G7)
            ...
            switch(regid) {
                ...
                // fuzz add regs
                case UC_SPARC_REG_NPC:
                    SPARC_CPU(uc, mycpu)->env.pc = *(uint32_t *)value - 4;
                    SPARC_CPU(uc, mycpu)->env.npc = *(uint32_t *)value;
                    // force to quit execution and flush TB
                    uc->quit_request = true;
                    //uc_emu_stop(uc);
                    uc->stop_request = true;
                    ...
                    break;
                case UC_SPARC_REG_PSR:
                    cpu_put_psr(&(SPARC_CPU(uc, mycpu)->env), *(uint32_t *)value);
                    break;
                case UC_SPARC_REG_WIM:
#if !defined(TARGET_SPARC64) || defined(TARGET_ABI32)
                    SPARC_CPU(uc, mycpu)->env.wim = *(uint32_t *)value;
#endif
                    break;
                case UC_SPARC_REG_TBR:
                    SPARC_CPU(uc, mycpu)->env.tbr = *(uint32_t *)value;
                    break;
                case UC_SPARC_REG_FSR:
                    SPARC_CPU(uc, mycpu)->env.fsr = *(uint32_t *)value;
                    break;
            }
        }
    }
