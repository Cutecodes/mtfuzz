...
static inline void fuzzer_log_cmp(
    uintptr_t pc, uint64_t Arg1, uint64_t Arg2, uint32_t attr) {
  
  if (cmp_headers == NULL || cmp_log == NULL) {
    return;
  }
  uintptr_t k = pc ;
  k = (k >> 4) ^ (k << 8);
  ...
  cmp_headers[k].id = pc;
  uint32_t hits = cmp_headers[k].hits;

  if (unlikely(cmp_headers[k].type != CMP_TYPE_INS)){
    cmp_headers[k].shape = 7;
    cmp_headers[k].type = CMP_TYPE_INS;
    hits = 0;
  }

  cmp_headers[k].hits = hits + 1;
  if (!cmp_headers[k].cnt)
    cmp_headers[k].cnt = cmp_counter++;

  cmp_headers[k].attribute = attr;
  if (hits >= cmp_map_h) {
    cmp_headers[k].overflow = 1;
  }
  
  hits &= cmp_map_h - 1;
  cmp_log[k*cmp_map_h + hits].v0 = Arg1;
  cmp_log[k*cmp_map_h + hits].v1 = Arg2;

}

static inline void fuzzer_log_cmp32(
    uintptr_t pc, uint32_t Arg1, uint32_t Arg2, uint32_t attr) {
  ...
  //printf("pc:%016x  op: %d op1:%lx op2:%lx\n",pc, attr,Arg1,Arg2);
  hits &= cmp_map_h - 1;
  cmp_log[k*cmp_map_h + hits].v0 = Arg1;
  cmp_log[k*cmp_map_h + hits].v1 = Arg2;
...
static void fuzzer_gen_log_cmp(TCGContext *tcg_ctx, target_ulong pc, TCGv arg1, TCGv arg2, enum op_type attr)
{
    ...
    TCGv_i32 tattr = tcg_const_i32(tcg_ctx, attr);

#if TARGET_LONG_BITS == 32
    TCGArg args[4] = { GET_TCGV_PTR(tcg_const_ptr(tcg_ctx, pc)),
                         GET_TCGV_I32(arg1),
                         GET_TCGV_I32(arg2),
                         GET_TCGV_I32(tattr) };
    tcg_gen_callN(tcg_ctx, fuzzer_log_cmp32, dh_retvar_void, 4, args);
#elif TARGET_LONG_BITS == 64
    TCGArg args[4] = { GET_TCGV_PTR(tcg_const_ptr(tcg_ctx, pc)),
                         GET_TCGV_I64(arg1),
                         GET_TCGV_I64(arg2),
                         GET_TCGV_I32(tattr) };
    tcg_gen_callN(tcg_ctx, fuzzer_log_cmp, dh_retvar_void, 4, args);
#else
    tcg_gen_callN(tcg_ctx, fuzzer_log_cmp, dh_retvar_void, 4, args);
#endif
    
    tcg_temp_free_i32(tcg_ctx, tattr);
}

static void fuzzer_gen_log_cmp32(TCGContext *tcg_ctx, target_ulong pc, TCGv_i32 arg1, TCGv_i32 arg2, enum op_type attr)
{
...
static void fuzzer_gen_log_cmp64(TCGContext *tcg_ctx, target_ulong pc, TCGv_i64 arg1, TCGv_i64 arg2, enum op_type attr)
{
...
static int fuzzer_get_cmp_type(CPUSPARCState *env, target_ulong pc) {
    // now just return 1
    return 1;
    
    unsigned int insn = cpu_ldl_code(env, pc);
    unsigned int opc, rd, xop;
    unsigned int cond;
    unsigned int ret = 0;
    ...
    xop = GET_FIELD(insn, 7, 9);
    
    if (xop == 2) {
        // int cmp
        switch(cond) {
            case 1:  case 9:  ret = OP_ICMP_E; break;
            case 2:  case 3:  ret = OP_ICMP_L; break;
            case 10: case 11: ret = OP_ICMP_G; break;
            case 12: case 13: ret = OP_UICMP_G; break;
            case 4:  case 5:  ret = OP_UICMP_L; break;
            default: break;
        }
    }else if (xop == 6) {
        // float cmp
...
static void sync_pc_to_cpustate(struct uc_struct *uc, TCGContext *tcg_ctx, target_ulong pc) {
    TCGv_ptr ptr = tcg_const_ptr(tcg_ctx, &(((CPUSPARCState *)uc->cpu->env_ptr)->pc));
#if TARGET_LONG_SIZE == 4
    TCGv tpc = tcg_const_i32(tcg_ctx, pc);
    tcg_gen_st_i32(tcg_ctx, tpc, ptr, 0);
#elif TARGET_LONG_SIZE == 8
    TCGv tpc = tcg_const_i64(tcg_ctx, pc);
    tcg_gen_st_i64(tcg_ctx, tpc, ptr, 0);
#else
...
/* Sync the guest PC to the CPUARMState */
static void sync_pc_npc_to_cpustate(struct uc_struct *uc, TCGContext *tcg_ctx, target_ulong pc) {
    TCGv_ptr ptr = tcg_const_ptr(tcg_ctx, &(((CPUSPARCState *)uc->cpu->env_ptr)->pc));
    TCGv_ptr nptr = tcg_const_ptr(tcg_ctx, &(((CPUSPARCState *)uc->cpu->env_ptr)->npc));
#if TARGET_LONG_SIZE == 4
    TCGv tpc = tcg_const_i32(tcg_ctx, pc);
    tcg_gen_st_i32(tcg_ctx, tpc, ptr, 0);
    TCGv tnpc = tcg_const_i32(tcg_ctx, pc+4);
    tcg_gen_st_i32(tcg_ctx, tnpc, nptr, 0);
#elif TARGET_LONG_SIZE == 8
    TCGv tpc = tcg_const_i64(tcg_ctx, pc);
    tcg_gen_st_i64(tcg_ctx, tpc, ptr, 0);
    TCGv tnpc = tcg_const_i64(tcg_ctx, pc+4);
    tcg_gen_st_i64(tcg_ctx, tnpc, nptr, 0);
#else
#error TARGET_LONG_SIZE undefined
#endif
...
static void disas_sparc_insn(CPUSPARCState *env, DisasContext * dc, unsigned int insn, bool hook_insn)
{
    TCGContext *tcg_ctx = dc->uc->tcg_ctx;
    ...
    sync_pc_to_cpustate(dc->uc, tcg_ctx, dc->pc);
...
#undef FMOVCC
                    ...
                    case 0x51: /* fcmps, V9 %fcc */
                        ...
                        fuzzer_gen_log_cmp32(tcg_ctx, dc->pc, cpu_src1_32, cpu_src2_32, OP_FCMP);
                        gen_op_fcmps(dc, rd & 3, cpu_src1_32, cpu_src2_32);
                        break;
                    case 0x52: /* fcmpd, V9 %fcc */
                        cpu_src1_64 = gen_load_fpr_D(dc, rs1);
                        cpu_src2_64 = gen_load_fpr_D(dc, rs2);
                        fuzzer_gen_log_cmp64(tcg_ctx, dc->pc, cpu_src1_64, cpu_src2_64, OP_FCMP);
                ...
                if (xop < 0x20) {
                    ...
                    case 0x0: /* add */
                        if (xop & 0x10) {
                            //fuzzer_gen_log_cmp(tcg_ctx, dc->pc, cpu_src1, cpu_src2, OP_IADD);
                    ...
                    case 0x1: /* and */
                        if (xop & 0x10)
                            fuzzer_gen_log_cmp(tcg_ctx, dc->pc, cpu_src1, cpu_src2, OP_AND);
                    ...
                    case 0x2: /* or */
                        if (xop & 0x10)
                            fuzzer_gen_log_cmp(tcg_ctx, dc->pc, cpu_src1, cpu_src2, OP_OR);
                    ...
                    case 0x3: /* xor */
                        if (xop & 0x10)
                            fuzzer_gen_log_cmp(tcg_ctx, dc->pc, cpu_src1, cpu_src2, OP_XOR);
                    ...
                    case 0x4: /* sub */
                        if (xop & 0x10) {
                            result_type = fuzzer_get_cmp_type(env, dc->pc+4); 
                            if (result_type) {
                                //fuzzer_gen_log_cmp(tcg_ctx, dc->pc, cpu_src1, cpu_src2, result_type | OP_ISUB);
                                fuzzer_gen_log_cmp(tcg_ctx, dc->pc, cpu_src1, cpu_src2, OP_ISUB);
                            }
...
//sync_pc_npc_to_cpustate(env->uc, tcg_ctx, tb->pc);
    ...
    do {
        ...
        disas_sparc_insn(env, dc, insn, true);
        num_insns++;
