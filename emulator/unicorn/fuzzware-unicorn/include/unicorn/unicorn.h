...
/*
  ...
  @mode: support block and edge etc.

  @return UC_ERR_OK on success, or other value on failure (refer to uc_err enum
   for detailed error).
*/
UNICORN_EXPORT
uc_err uc_fuzzer_init_cov(uc_engine *uc, void *bitmap_region, uint32_t bitmap_size, uint32_t mode);
...
/*
  Snapshot coverage recording.

  @uc: handle returned by uc_open()

  @return the pointer of the buffer on success, or NULL on failure.
*/
UNICORN_EXPORT
void* uc_fuzzer_snapshot_cov(uc_engine *uc);

/*
  Restore coverage recording.

  @uc: handle returned by uc_open()
  @buffer: the snapshot buffer.
...
UNICORN_EXPORT
uc_err uc_fuzzer_restore_cov(uc_engine *uc, void* buffer);


UNICORN_EXPORT
uc_err uc_fuzzer_get_cov_prev_loc(uc_engine *uc, void* val);

UNICORN_EXPORT
uc_err uc_fuzzer_set_cov_prev_loc(uc_engine *uc, const void* val);

/*
  Initialize recording code cmplog.

  @uc: handle returned by uc_open()
  @headers: the buffer to hold the headers
  @logs: the buffer to hold the data logs
  @w: the width of data logs
  @h: the hight of data logs
  @mode: different mode support

  @return UC_ERR_OK on success, or other value on failure (refer to uc_err enum
   for detailed error).
*/
UNICORN_EXPORT
uc_err uc_fuzzer_init_cmplog(uc_engine *uc, void *headers, void* logs, uint32_t w, uint32_t h, uint32_t mode);

/*
  Reset cmplog recording.
...
*/
UNICORN_EXPORT
uc_err uc_fuzzer_reset_cmplog(uc_engine *uc, int do_clear);

/*
  Snapshot cmplog recording.
...
*/
UNICORN_EXPORT
void* uc_fuzzer_snapshot_cmplog(uc_engine *uc);

/*
  Restore cmplog recording.
...
UNICORN_EXPORT
uc_err uc_fuzzer_restore_cmplog(uc_engine *uc, void* buffer);
...
/*
  ...
  @uc: handle returned by uc_open()
  @sensitive: enable context sensitive when set
  @mode: different mode support
...
*/
UNICORN_EXPORT
uc_err uc_fuzzer_init_context(uc_engine *uc, uint32_t sensitive ,uint32_t mode);

/*
  Reset context recording.
...
*/
UNICORN_EXPORT
uc_err uc_fuzzer_reset_context(uc_engine *uc, int do_clear);

/*
  Snapshot context recording.
...
*/
UNICORN_EXPORT
void* uc_fuzzer_snapshot_context(uc_engine *uc);

/*
  Restore context recording.
...
UNICORN_EXPORT
uc_err uc_fuzzer_restore_context(uc_engine *uc, void* buffer);
