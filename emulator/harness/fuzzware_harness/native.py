...
# ARCH
def init_arch(uc, arch):
    global native_lib
    assert native_lib.init_arch(uc._uch, arch) == 0
...
def fuzzer_main(uc, input_path, output_path, tmout=10000):
    # uc_err fuzzer_main(uc_engine *uc, char *p_input_path, char *p_output_path, uint32_t tmout)


    native_lib.fuzzer_main(uc._uch, input_path.encode(), output_path.encode(), tmout)

def get_fuzz(uc, addr, size):
    
    buf = (ctypes.c_char * size)()
    
    if native_lib.get_fuzz(uc._uch, addr, size, buf) == 0:
        return buf.raw
    else:
        return None
...
def register_py_handled_mmio_ranges(uc, python_handled_range_starts, python_handled_range_ends):
    ...
    assert native_lib.register_py_handled_mmio_ranges(uc._uch, mmio_cb_wrapper, starts_arr, ends_arr, len(python_handled_range_ends), user_data) == 0
...
def add_ignored_mmio_addresses(uc, addresses, pcs):
    addrs_arr = (ctypes.c_int64 * len(addresses))(*addresses)
    pcs_arr = (ctypes.c_uint32 * len(pcs))(*pcs)

    assert native_lib.add_ignored_mmio_addresses(uc._uch,
...
def init_nvic(uc, vtor, num_vecs, interrupt_limit=DEFAULT_MAX_INTERRUPTS, disabled_interrupts=()):
    ...
    assert native_lib.arch_init_nvic(uc._uch, vtor, num_vecs, interrupt_limit, len(disabled_interrupts), disabled_interrupts_arr) == 0
...
def nvic_set_pending(vec_num):
    global native_lib
    native_lib.arch_ic_set_pending(vec_num)
...
def init_systick(uc, reload_val):
    ...
    assert native_lib.arch_init_systick(uc._uch, reload_val) == 0
...
def init(uc, mmio_regions, exit_at_bbls, exit_at_hit_num, do_print_exit_info, fuzz_consumption_timeout=DEFAULT_FUZZ_CONSUMPTION_TIMEOUT, instr_limit=DEFAULT_BASIC_BLOCK_LIMIT, cov_mode=0, cmp_mode=0, use_stream=False, fix_checksum_mode=False, smart_stream=False, its_mode=False):
    global native_lib
    global mmio_cb_wrapper
    ...
    # ARCH
    _setup_prototype(native_lib, "init_arch", ctypes.c_int, uc_engine)
    # init_fuzz(uc_engine *uc, exit_hook_t p_exit_hook, int p_num_mmio_regions, uint64_t *p_mmio_starts, uint64_t *p_mmio_ends, int p_num_ignored_mmio_addresses, uint64_t *p_ignored_mmio_addressed, uint32_t *p_ignored_address_pcs, uint32_t num_exit_at_bbls, uint64_t *exit_at_bbls, uint32_t p_exit_at_hit_limit, int p_do_print_exit_info, uint64_t p_fuzz_consumption_timeout, uint32_t p_instr_limit,uint32_t p_cov_mode, uint32_t p_cmp_mode, uint32_t p_use_stream)
    _setup_prototype(native_lib, "init_fuzz", ctypes.c_int, uc_engine, ctypes.c_void_p, ctypes.c_int, ctypes.c_void_p, ctypes.c_void_p, ctypes.c_int, ctypes.c_void_p, ctypes.c_void_p,
             ctypes.c_uint32, ctypes.c_void_p, ctypes.c_uint32, ctypes.c_uint, ctypes.c_uint64, ctypes.c_uint64, ctypes.c_uint32, ctypes.c_uint32, ctypes.c_uint32)
    ...
    _setup_prototype(native_lib, "get_fuzz", ctypes.c_int, uc_engine, ctypes.c_uint64, ctypes.c_uint32, ctypes.c_void_p)
    ...
    # extern uc_err register_py_handled_mmio_ranges(uc_engine *uc, uc_cb_hookmem_t py_callback, uint64_t *starts, uint64_t *ends, int num_ranges, void* user_data);
    _setup_prototype(native_lib, "register_py_handled_mmio_ranges", ctypes.c_int, uc_engine, ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p, ctypes.c_int, ctypes.c_void_p)
    # extern uc_err set_ignored_mmio_addresses(uint64_t *addresses, uint32_t *pcs, int num_addresses);
    _setup_prototype(native_lib, "add_ignored_mmio_addresses", ctypes.c_int, ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p, ctypes.c_int)
    ...
    _setup_prototype(native_lib, "arch_init_nvic", ctypes.c_int, uc_engine, ctypes.c_uint, ctypes.c_uint, ctypes.c_uint32, ctypes.c_uint32, ctypes.c_void_p)
    # extern void nvic_set_pending(int num)
    _setup_prototype(native_lib, "arch_ic_set_pending", ctypes.c_int, ctypes.c_int)
    ...
    _setup_prototype(native_lib, "arch_init_systick", ctypes.c_int, uc_engine, ctypes.c_uint32)
    ...
    # fuzz_main
    # uc_err fuzzer_main(uc_engine *uc, char *p_input_path, char *p_output_path, uint32_t tmout)
    _setup_prototype(native_lib, "fuzzer_main", ctypes.c_int, uc_engine, ctypes.c_char_p, ctypes.c_char_p, ctypes.c_uint32)
    ...
    assert native_lib.init_fuzz(uc._uch, exit_cb, len(mmio_regions), mmio_region_starts_arr, mmio_region_ends_arr, 0, None, None, num_exit_at_bbls, exit_at_bbls_arr, exit_at_hit_num, do_print_exit_info, fuzz_consumption_timeout, instr_limit, cov_mode, cmp_mode, use_stream, fix_checksum_mode, smart_stream, its_mode) == 0
