...
static void interrupt_trigger_tick_block_hook(uc_engine *uc, uint64_t address, uint32_t size, void *user_data) {
    ...
    } else if (trigger->curr_pends)
    {
        ...
        arch_ic_set_pending(uc, trigger->irq, false);
        ++trigger->curr_pends;
        #ifdef DEBUG_INTERRUPT_TRIGGERS
        ...
        switch(trigger->fuzz_mode) {
            ...
            case IRQ_FUZZ_MODE_FUZZ_ENABLED_IRQ_INDEX:
                ...
                num_enabled = arch_get_num_enabled();
                if (num_enabled)
                {
                    ...
                    if (num_enabled != 1)
                    {
                        if(get_fuzz(uc, address, sizeof(irq_ind), (void *)&irq_ind)) {
                            return;
                        }
                    }
                    trigger->irq = arch_nth_enabled_irq_num(irq_ind);
            ...
            case IRQ_FUZZ_MODE_ROUND_ROBIN:
                if (arch_get_num_enabled()) {
                    trigger->irq = arch_nth_enabled_irq_num(trigger->round_robin_index++);
                    #ifdef DEBUG_INTERRUPT_TRIGGERS
        ...
        if(trigger->trigger_mode == IRQ_TRIGGER_MODE_TIME_FUZZED) {
            ...
            if(get_fuzz(uc, address,  sizeof(time_fuzzer_choice), (void *)&time_fuzzer_choice)) {
                return;
            }
