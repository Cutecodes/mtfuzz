...
#ifdef FERMCOV
    if (excp_prev_loc) {
        uc_fuzzer_set_cov_prev_loc(uc, &excp_prev_loc);
        excp_prev_loc = 0;
    }
#endif
...
#ifdef FERMCOV
    uc_fuzzer_get_cov_prev_loc(uc, &excp_prev_loc);
    unsigned long tmp = 0;
...
#ifdef FERMCOV
    char *result = malloc(size + PAGE_SIZE + sizeof(excp_prev_loc));
#else
...
#ifdef FERMCOV
    memcpy(result + size + PAGE_SIZE, &excp_prev_loc, sizeof(excp_prev_loc));
#endif
    return result;
}
...
#ifdef FERMCOV
    memcpy(&excp_prev_loc, ((char *) snapshot) + sizeof(nvic) + PAGE_SIZE,  sizeof(excp_prev_loc));
#endif
}
...
uc_err arm_init_nvic(uc_engine *uc, uint32_t vtor, uint32_t num_irq, uint32_t p_interrupt_limit, uint32_t num_disabled_interrupts, uint32_t *disabled_interrupts) {
    #ifdef DEBUG_NVIC
...
uint16_t arm_get_num_enabled() {
    return nvic.num_enabled;
}

uint8_t arm_nth_enabled_irq_num(uint8_t n) {
    return nvic.enabled_irqs[n % nvic.num_enabled];
}

void arm_nvic_set_pending(uc_engine *uc, uint32_t num, int delay_activation) {
