...
uc_err arm_init_nvic(uc_engine *uc, uint32_t vtor, uint32_t num_irq, uint32_t interrupt_limit, uint32_t num_disabled_interrupts, uint32_t *disabled_interrupts);
...
void arm_nvic_set_pending(uc_engine *uc, uint32_t num, int skip_current_instruction);
