...
from unicorn import (UC_ARCH_SPARC, UC_MODE_SPARC32, UC_MODE_BIG_ENDIAN, UC_ARCH_ARM, UC_MODE_MCLASS, UC_MODE_THUMB, Uc,UC_HOOK_BLOCK,UC_HOOK_CODE)
from unicorn.arm_const import UC_ARM_REG_PC, UC_ARM_REG_SP,UC_ARM_REG_PC,UC_ARM_REG_LR,UC_ARM_REG_CPSR,UC_ARM_REG_PRIMASK,UC_ARM_REG_BASEPRI,UC_ARM_REG_CURR_SP_MODE_IS_PSP,UC_ARM_REG_R2,UC_ARM_REG_R3
...
lastblock = 0
block_count = 0
arm_md = Cs(CS_ARCH_ARM, CS_MODE_THUMB)
md = Cs(CS_ARCH_SPARC,CS_MODE_V9)
...
def hook_block(mu, address, size, user_data):
    ...
    npc = mu.reg_read(UC_SPARC_REG_NPC)
    #if address >  0x40007958  and address < 0x40007Ef4:
    #    print("[HOOK BLOCK]: %s %s %s"%(hex(address),hex(pc),hex(npc)))    	
    #return
    print("[HOOK BLOCK]: %s %s %s"%(hex(address),hex(pc),hex(npc)))
    #print("[HOOK BLOCK]: %s %s"%(hex(address),hex(pc)))
    times = times + 1
    #if times == 13:
       # print("#############snap##############")
        #trigger_snapshotting(mu)
    #if times == 100:
     #   print("#############restore###########")
        
     #   trigger_restore(mu)
      #  pc = mu.reg_read(UC_SPARC_REG_PC)
       # mu.emu_start(pc-4, ADDRESS + len(SPARC_CODE32))
    #if times == 120:
      #  mu.emu_stop()
    #if times % 99 == 98:
        #nvic_set_pending(mu,8)
           
    
def hook_code(mu, address, size, user_data):
    Value = mu.mem_read(address,size)
    psr = mu.reg_read(UC_SPARC_REG_PSR)
    wim = mu.reg_read(UC_SPARC_REG_WIM)
    sp = mu.reg_read(UC_SPARC_REG_SP)
    fp = mu.reg_read(UC_SPARC_REG_FP)
    pc = mu.reg_read(UC_SPARC_REG_PC)
    npc = mu.reg_read(UC_SPARC_REG_NPC)
    addr = int(address)
    for item in md.disasm(Value, 0):
        
        dic = {'PC':hex(pc),'NPC':hex(npc),'PSR:':hex(psr),'WIM':bin(wim),"FP":hex(fp),"SP":hex(sp),"Addr": hex(addr) , "OpCode": item.mnemonic + " " + item.op_str}
        print(dic)
        return 
    dic = {'PC':hex(pc),'NPC':hex(npc),'PSR:':hex(psr),'WIM':bin(wim),"FP":hex(fp),"SP":hex(sp),"Addr": hex(addr)}
    print(dic)
def hook_arm_code(mu, address, size, user_data):
    global block_count
    ...
    primask = mu.reg_read(UC_ARM_REG_PRIMASK)
    basepri = mu.reg_read(UC_ARM_REG_BASEPRI)
    is_psp = mu.reg_read(UC_ARM_REG_CURR_SP_MODE_IS_PSP)

    for item in arm_md.disasm(Value, 0):
        addr = int(address)
        dic = {'PC':hex(pc),"LR":hex(lr), "CPSR":hex(cpsr),"SP":hex(sp),"PRIMASK":hex(primask),"BASEPRI":hex(basepri),"IS_PSP":hex(is_psp),"Addr": hex(addr) , "OpCode": item.mnemonic + " " + item.op_str}
        print(dic)
        r2 = mu.reg_read(UC_ARM_REG_R2)
        r3 = mu.reg_read(UC_ARM_REG_R3)
    
        dic={'r2':hex(r2),'r3':hex(r3)}
        print(dic)
        return
    print({'PC':hex(pc),"LR":hex(lr),"CPSR":hex(cpsr), "SP":hex(sp),"PRIMASK":hex(primask), "BASEPRI":hex(basepri), "IS_PSP":hex(is_psp), "Addr": hex(address)})
def hook_arm_block(mu, address, size, user_data):
    global lastblock,block_count
    if lastblock == address:
        block_count += 1
        return
    print("[HOOK BLOCK]: %s %s"%(hex(lastblock),block_count))
    lastblock = address
    block_count = 0    
    pc = mu.reg_read(UC_ARM_REG_PC)
    lr = mu.reg_read(UC_ARM_REG_LR)
    print("[HOOK BLOCK]: %s %s %s"%(hex(address),hex(pc),hex(lr)))  
...
def configure_unicorn(args):
    ...
    if arch == "sparc":
        ...
        uc.reg_write(UC_SPARC_REG_PC, config['entry_point'])
        # The stack pointer is aligned during CPU reset
        uc.reg_write(UC_SPARC_REG_SP, config['initial_sp'] & 0xfffffffc)
    elif arch == "arm":
        # Set the program entry point and stack pointer
    ...
    # Native arch setup
    if arch == "arm":
        native.init_arch(uc, UC_ARCH_ARM)
    elif arch == "sparc":
        native.init_arch(uc, UC_ARCH_SPARC)
    else:
        logger.error("Not support arch!")
        sys.exit(1) 
    ...
    # Native mmio fuzzing
    native.init(uc, mmio_ranges, exit_at_bbls, args.exit_at_hit_num, args.print_exit_info, args.fuzz_consumption_timeout, args.basic_block_limit,args.cov_mode,args.cmp_mode,args.use_stream,args.fix_checksum_mode,args.smart_stream, args.its_mode)
    ...
    if args.gdb_port != 0:
        ...
        uc.gdb = None
    #uc.hook_add(UC_HOOK_CODE, hook_arm_code, 0x8000000, 0x8000000+0x0d000)
    #uc.hook_add(UC_HOOK_BLOCK, hook_arm_block, 0x8000000,0x8000000+0x0d000)
    #uc.hook_add(UC_HOOK_CODE, hook_code, begin=0x40000000, end=0x40010000)
    #uc.hook_add(UC_HOOK_BLOCK, hook_block, begin=0x40000000, end=0x40010000)
    return uc
...
def populate_parser(parser):
    ...
    # fuzz settings
    parser.add_argument('--cov_mode', type=int, default=0 , help="Controls the coverage instrumentation used for feedback by the fuzzer")
    parser.add_argument('--cmp_mode', type=int, default=0 , help="Controls the cmplog instrumentation used for feedback by the fuzzer")
    parser.add_argument('--use_stream', default=False , action="store_true", help="Controls whether to use stream")
    parser.add_argument('--fix_checksum_mode', default=False , action="store_true", help="Controls whether to use fix_checksum_mode")
    parser.add_argument('--smart_stream', default=False , action="store_true", help="Controls whether to use smart stream")
    parser.add_argument('--its_mode', default=False , action="store_true", help="Controls whether to use its")
    parser.add_argument('--do_emu', default=False , action="store_true", help="emulate one testcase or fuzz")
    parser.add_argument('-i', '--input_dir', help="The fuzzer input path")
    parser.add_argument('-o', '--output_dir', help="The fuzzer output path")
    parser.add_argument('--tmout', type=int, default=10000, help='fuzzer tmout')
...
def main():
    ...
    if args.do_emu:
        ...
        if args.input_dir is None:
            logger.error("input dir is not correctly set")
            sys.exit(1)
        if args.output_dir is None:
            logger.error("output dir is not correctly set")
            sys.exit(1)
        
        native.fuzzer_main(uc, args.input_dir, args.output_dir, tmout)
